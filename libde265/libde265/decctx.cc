                                                          ...
                                                          slice_unit* sliceunit)
{
  ...
  */
  ...
  if (sliceunit->shdr->slice_segment_address >= (int)imgunit->img->get_pps().CtbAddrRStoTS.size()) {
    return DE265_ERROR_CTB_OUTSIDE_IMAGE_AREA;
  }


  class thread_context tctx;
...
de265_error decoder_context::process_reference_picture_set(slice_segment_header* hdr)
{
    ...
    */
    ...
    for (int i=0;i<dpb.size();i++) {
      ...
      if (img->PicState != UnusedForReference &&
          ...
          (uint32_t) img->removed_at_picture_id > (uint32_t)img->get_ID()) {
  ...
  for (int i=0;i<dpb.size();i++)
    if (i>=(int)picInAnyList.size() || !picInAnyList[i])        // no reference
      {
        de265_image* dpbimg = dpb.get_image(i);
        if (dpbimg != img &&  // not the current picture
            (uint32_t)dpbimg->removed_at_picture_id > img->get_ID()) // has not been removed before
          {
