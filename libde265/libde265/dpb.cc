...
bool decoded_picture_buffer::has_free_dpb_picture(bool high_priority) const
{
  ...
  if ((int)dpb.size() < max_images_in_DPB) return true;
                                      ...
                                      de265_PTS pts, void* user_data, bool isOutputImage)
{
  ...
  if ((int)dpb.size() > norm_images_in_DPB &&           // buffer too large
      free_image_buffer_idx != (int)dpb.size()-1 &&     // last slot not reused in this alloc
  ...
  if (free_image_buffer_idx == -DE265_ERROR_IMAGE_BUFFER_FULL) {
    free_image_buffer_idx = (int)dpb.size();
...
void decoded_picture_buffer::pop_next_picture_in_output_queue()
{
  ...
  for (int i=0;i<(int)image_output_queue.size();i++) {
